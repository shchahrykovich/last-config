...
export type AuthHandlerForProjectsWithParams<T, P = unknown> = (currentUser: User,
                                                     db: PrismaClient,
                                                     req: NextRequest,
                                                     projectId: number,
                                                     {params}: { params: Promise<P> }) => Promise<T>;
export function authMiddlewareForProjects<T, P>(handler: AuthHandlerForProjectsWithParams<T, P>) {
    return async (req: NextRequest, {params}: { params: Promise<P> }) => {
        let currentUser: User | null = null;
        ...
        try {
            ...
            return loggingStore.run({
                ...
                const projectId = parseInt(p.id, 10);
                ...
                return await handler(currentUser!, db, req, projectId, {params});
            });
        } catch (error) {
            return createErrorResponse(error, 'generic_error');
        }
