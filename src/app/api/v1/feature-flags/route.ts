...
import { GetFeatureFlagRequestSchema, type GetFeatureFlagResponse } from './dto'
...
export const GET = secretApiKeyAuthMiddleware(async (context, db, req, { params }: { params: Promise<Params> }) => {
    try {
        ...
        // Get feature flags with priority-based lookup
        const featureFlags = await featureFlagService.getFeatureFlagsByNamesWithPriority(
            context.projectId,
            context.tenantId,
            validatedData.names,
