...
const FeatureFlagsApiExamples = () => {
    ...
    const examples: Example[] = [
        {
            title: 'Get feature flags (secret)',
            key: 'getFeatureFlags',
            code: {
                curl: `curl -X GET ${domain}/api/v1/feature-flags?name={name} \\
  -H "Authorization: sk_{public}_{private}"`,
                typescript: `const res = await fetch("${domain}/api/v1/feature-flags?name={name}", {
  headers: {
    Authorization: "sk_{public}_{private}",
        ...
        {
            title: `Get public feature flags${!publicKey ? ' (Create a public API key first)' : ''}`,
            key: 'getPublicFeatureFlags',
            code: {
                curl: `curl -X GET ${domain}/api/v1/public/feature-flags?name={name} \\
  -H "Authorization: ${publicKey || 'pk_{public}'}"`,
                typescript: `const res = await fetch("${domain}/api/v1/public/feature-flags?name={name}", {
  headers: {
...
export default FeatureFlagsApiExamples;
